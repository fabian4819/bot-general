...
export async function handleCommand(text: string, spreadsheetId?: string): Promise<string | null> {
  ...
  if (cmd === '/help' || cmd === '/bantuan') {
    return [
      ...
      '  /saldo awal 0',
      '',
      'Setelah itu catat transaksi dengan awalan /:',
      '  /makan siang 25rb',
      '  /gaji masuk 5jt',
      '  /bayar listrik 450.000',
      '',
      'Pesan tanpa awalan / akan diabaikan.',
      '',
      'Perintah cashflow:',
      ...
      '',
      '🧾 *Invoice*',
      'Ketik /invoice untuk melihat template invoice.',
      'Buat invoice dalam 1 pesan:',
      '```',
      '/invoice',
      'Bill To: Nama Klien',
      'Campaign: Nama Campaign',
      'Item: Nama | Deskripsi | Qty | Rate',
      'Item 2: Nama 2 | Deskripsi | Qty | Rate',
      '',
      'Brand: Nama Brand (opsional)',
      'Mastersheet (opsional)',
      ...
      'Pigeon Micro | 1x VT + IG Reels | 15 | 250rb',
      '```',
      '',
      'Alias invoice: /inv',
      '',
